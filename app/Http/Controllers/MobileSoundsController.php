<?php

namespace App\Http\Controllers;

use App\Mobile\Sound;
use Illuminate\Http\Request;

class MobileSoundsController extends ApiController
{

    /**
     * Display a listing of the resource.
     *
     * @param null $id
     * @return \Illuminate\Http\Response
     */
    public function index(Request $request)
    {

        if ($request->method() != 'GET')
        {

            $this->respondMethodNotAllowed();

        }

        $ids = $request->input('ids');
        if ($ids)
        {

            return $this->showMutliple($ids);

        }

        $limit = $request->input('limit') ?: 12;
        if ($limit > static::LIMIT_MAX) return $this->respondForbidden('Invalid limit', 'You have requested too many sounds. Please set a smaller limit.');
        
        $all = Sound::paginate($limit);
        return response()->collection($all, new \App\Http\Transformers\MobileSoundTransformer);

    }

    /**
     * Display the specified resource.
     *
     * @param  \App\Mobile\Sound  $sound
     * @return \Illuminate\Http\Response
     */
    public function show(Request $request, $soundId)
    {

        if ($request->method() != 'GET')
        {

            $this->respondMethodNotAllowed();

        }

        try
        {
            if (intval($soundId) <= 0)
            {
                return $this->respondInvalidSyntax('Invalid identifier', "The sound identifier should be a number. Please ensure you're passing the correct source identifier and try again.");
            }

            $item = Sound::find($soundId);

            if (!$item)
            {
                return $this->respondNotFound('Sound not found', "The sound you requested cannot be found. Please ensure you're passing the source identifier and try again.");
            }

            return response()->item($item, new \App\Http\Transformers\MobileSoundTransformer);
        }
        catch(\Exception $e)
        {
            return $this->respondFailure();
        }
        
    }

    public function showMutliple($ids = '')
    {

        $ids = explode(',',$ids);
        if (count($ids) > static::LIMIT_MAX)
        {
            
            return $this->respondForbidden('Invalid number of ids', 'You have requested too many ids. Please send a smaller amount.');
            
        }
        $all = Sound::find($ids);
        return response()->collection($all, new \App\Http\Transformers\MobileSoundTransformer);
        
    }

}
