...
class EventsController extends ApiController
{
    ...
    public function show(Request $request, $eventId)
    {
        ...
        try
        {
            if (intval($eventId) <= 0)
            {
                return $this->respondInvalidSyntax('Invalid identifier', "The event identifier should be a number. Please ensure you're passing the correct source identifier and try again.");
            }
            ...
            if (!$item)
            {
                return $this->respondNotFound('Event not found', "The event you requested cannot be found. Please ensure you're passing the source identifier and try again.");
            }

            return response()->item($item, new \App\Http\Transformers\EventTransformer);
        }
        catch(\Exception $e)
        {
